...
func isOnline(checkURL string, skipTlsVerify bool, timeout int) (ret bool) {
	// Block SiYuan servers for security, allow only OpenAI
	if strings.Contains(checkURL, "b3logfile.com") || 
	   strings.Contains(checkURL, "liuyun.io") || 
	   strings.Contains(checkURL, "ld246.com") {
		logging.LogInfof("Blocked connection to SiYuan server: %s", checkURL)
		return false
	}
	c := req.C().
